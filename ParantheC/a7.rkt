...
; step 7: define-label and program counter DONE
; step 8: add trampoline and dismount it with empty continuation DONE
; step 9: comment out unnecessary lines DONE
...
(define-label apply-k
    (union-case k-cps continuation
        ;step 8: jumpout when execute empty-k continuation
        [(empty-k jump-out) (dismount-trampoline jump-out)]
        [(app-clos rand^ env^ k^)
            (begin 
...
(define-label main
    (begin 
        ...
        (set! pc value-of-cps)
        (mount-trampoline continuation_empty-k k-cps pc)
        (printf "Result is: ~s\n" v)))
