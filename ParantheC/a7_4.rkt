        ...
        (union-case exp expr
            ...
            [(if test conseq alt) 
                (value-of-cps test env
                    (continuation_if conseq alt env k))]
            [(letcc body) 
                (value-of-cps body (environment_extend-env k env) k)]
            [(throw k-exp v-exp) 
                (value-of-cps k-exp env 
                    (continuation_throw v-exp env))]
            [(let e body) 
                (value-of-cps e env
                    (continuation_let body env k))]
            [(var y) (apply-env env y k)]
            [(lambda body) 
                (apply-k k (closure_clos body env))]
            [(app rator rand) 
                (value-of-cps rator env
                    (continuation_app-clos rand env k))])))
...
(define apply-k
    (lambda (k v)
        (union-case k continuation
            [(empty-k) v]
            [(app-clos rand^ env^ k^) (value-of-cps rand^ env^ (continuation_app-arg v k^))]
            [(app-arg clos^ k^) (apply-closure clos^ v k^)]
            [(let body^ env^ k^) (value-of-cps body^ (environment_extend-env v env^) k^)]
            ...
            [(mult1 x2^ env^ k^) (value-of-cps x2^ env^ (continuation_mult2 v k^))]
...
(define apply-closure
    (lambda (clos a k)
        (union-case clos closure
            [(clos body env) (value-of-cps body (environment_extend-env a env) k)])))
